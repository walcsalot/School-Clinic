...
const EmergencyButton = ({ user }) => {
  ...
  const confirmEmergency = async () => {
    ...
    try {
      ...
      await addDoc(collection(db, "emergencies"), {
        ...
        notes: "Emergency assistance requested by employee",
        resolved: false
      });

      toast.success(`Emergency alert sent from ${fullLocation}! Help is on the way.`);
  ...
  return (
    <>
      {/* Emergency Button - Now smaller for mobile */}
      <div className="fixed bottom-6 right-6 z-50">
        <button
          onClick={handleEmergencyClick}
          disabled={isLoading || isActivated}
          className={`
            relative flex items-center justify-center p-4 rounded-full shadow-xl 
        ...
        >
          <div className="flex flex-col items-center">
            <svg 
              ...
              className="h-8 w-8 text-white"  // Reduced from h-12 w-12
              fill="none" 
              viewBox="0 0 24 24" 
              stroke="currentColor"
            >
            ...
            <span className="text-white font-bold mt-1 text-sm">  {/* Reduced text size and margin */}
